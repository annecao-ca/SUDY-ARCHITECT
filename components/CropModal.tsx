  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-gray-800 rounded-lg shadow-2xl p-4 w-full max-w-4xl h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <h2 className="text-xl font-bold text-white mb-4">{t('cropModal.title')}</h2>
        ...
        <div className="flex justify-end gap-4 mt-4">
            <button onClick={onClose} className="px-6 py-2 text-sm font-medium bg-gray-600 rounded-md hover:bg-gray-500 text-white">{t('cropModal.cancel')}</button>
            <button onClick={handleSave} className="px-6 py-2 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700">{t('cropModal.save')}</button>
        </div>
      </div>
    </div>
  );
