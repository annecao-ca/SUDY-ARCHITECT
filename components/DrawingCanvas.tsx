  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="flex flex-wrap items-center justify-center gap-2 p-2 bg-gray-100 dark:bg-gray-900/50 rounded-lg">
         <button onClick={() => setTool('brush')} className={toolButtonClass('brush')} title={t('drawingCanvas.brush')}><PaintBrushIcon className="w-5 h-5"/></button>
         <button onClick={() => setTool('eraser')} className={toolButtonClass('eraser')} title={t('drawingCanvas.eraser')}><EraserIcon className="w-5 h-5"/></button>
         ...
         <button onClick={undo} disabled={historyIndex <= 0} className={historyButtonClass(historyIndex <= 0)} title={t('drawingCanvas.undo')}><UndoIcon className="w-5 h-5"/></button>
         <button onClick={redo} disabled={historyIndex >= history.length - 1} className={historyButtonClass(historyIndex >= history.length - 1)} title={t('drawingCanvas.redo')}><RedoIcon className="w-5 h-5"/></button>
         <button onClick={clearCanvas} className="p-2 rounded-md bg-red-500/20 text-red-600 dark:text-red-400 hover:bg-red-500/40" title={t('drawingCanvas.clear')}><TrashIcon className="w-5 h-5"/></button>
      </div>
    </div>
  );
});
